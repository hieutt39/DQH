...
fc.datepickerLang = function(langCode, dpLangCode, dpOptions) {
	...
	// get the FullCalendar internal option hash for this language. insert if necessary
...
// Given the scrollbar widths of some other container, insert borders/margins on rowEls in order to match the left
...
// Create an object that has the given prototype. Just like Object.insert
...
// called upon a class to insert a subclass
...
var MouseFollower = Class.extend({
    ...
    /* A utilities class for rendering <tr> rows.
----------------------------------------------------------------------------------------------------------------------*/
...
Grid.mixin({
	...
	// Only returns segments that successfully rendered.
	// A utilities that subclasses may use.
	...
	startExternalDrag: function(el, ev, ui) {
		var _this = this;
		var meta = getDraggedElMeta(el); // extra data about event drop, including possible event to insert
		var dragListener;
	...
	// Generic utilities for generating the HTML classNames for an event segment's element
	...
	// will insert an array of ranges that span the time *not* covered by the given event.
...
var DayGrid = Grid.extend({
	...
	breakOnWeeks: null, // should insert a new row for each week? set by outside view
...
DayGrid.mixin({
	...
	renderSegRow: function(row, rowSegs) {
		...
		function emptyCellsUntil(endCol) {
			while (col < endCol) {
				// try to grab a cell from the level above and extend its rowspan. otherwise, insert a fresh cell
		...
		for (i = 0; i < levelCnt; i++) { // iterate through all levels
			...
			if (levelSegs) {
				for (j = 0; j < levelSegs.length; j++) { // iterate through segments in level
					...
					// insert a container that occupies or more columns. append the event element.
	...
	buildSegLevels: function(segs) {
		...
		for (i = 0; i < segs.length; i++) {
			...
			// insert new level array if needed and append segment
...
var View = fc.View = Class.extend({
	...
	function renderView(delta, viewType) {
		...
		// if viewType changed, or the view was never created, insert a fresh view
	...
	// Gets information about how to insert a view
...
var BasicView = fcViews.basic = View.extend({
	...
	numberCellHtml: function(cell) {
		...
		if (!this.dayNumbersVisible) { // if there are week numbers but not day numbers
			return '<td/>'; //  will insert an empty space above events :(
		}
...
fcViews.agenda = View.extend({ // AgendaView
	...
	// Builds the HTML skeleton for the view.
	// The day-grid and time-grid utilities will render inside containers defined by this HTML.
	renderHtml: function() {
		return '' +
			'<table>' +
				'<thead>' +
