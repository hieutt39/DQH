...
!function (a, b) {
    ...
    c = function () {
        ...
        return r = {
            buttons: {
                holder: '<div class="small-box-buttons">{{buttons}}</div>',
                submit: '<button type="submit" class="alertify-button alertify-button-ok btn btn-sm btn-flat btn-default" id="alertify-ok">{{ok}}</button>',
                ok: '<button class="alertify-button alertify-button-ok btn btn-sm btn-flat btn-warning" id="alertify-ok">{{ok}}</button>',
                cancel: '<button class="alertify-button alertify-button-cancel btn btn-flat btn-sm btn-primary" id="alertify-cancel">{{cancel}}</button>'
            },
            ...
            build: function (a) {
                ...
                switch (b += '<div class="alertify-dialog box box-primary">', b += '<a id="alertify-resetFocusBack" class="alertify-resetFocus" href="#">Reset Focus</a>', "none" === q.buttonFocus && (b += '<a href="#" id="alertify-noneFocus" class="alertify-hidden"></a>'), "prompt" === c && (b += '<div id="alertify-form">'), b += '<article class="alertify-inner inner">', b += r.message.replace("{{message}}", d), "prompt" === c && (b += r.input), b += "</article>",b += r.buttons.holder, "prompt" === c && (b += "</div>"), b += '<a id="alertify-resetFocus" class="alertify-resetFocus" href="#">Reset Focus</a>', b += "</div>", c) {
                    case"confirm":
            ...
            init: function () {
                ...
                /*hieutt*/
                //var top = (($(window).height() / 2) - ($(l).outerHeight() / 2))-100;
                var left = (($(window).width() / 2) - ($(l).outerWidth() / 2));
                if( left < 0 ) left = 0;
                l.setAttribute('style', 'left:'+left+'px;');
            },
            log: function (a, b, c) {
                var d = function () {
