...
from postman2py.extractors import extract_dict_from_raw_headers, extract_dict_from_headers, \
    extract_dict_from_raw_mode_data, format_object, extract_dict_from_formdata_mode_data, exctact_dict_from_files, \
    extract_dict_from_urlencoded
...
class PostPython:
    ...
    def get_collection_request(self):
        collections = []
        for fol in self.__folders.values():
            collections.append(fol.get_collection_request())
...
class PostCollection:
    ...
    def get_collection_request(self):
        collections = []
        for req in self.__requests.keys():
            collections.append("post_python.{COLLECTION}.{REQUEST}()".format(
                COLLECTION=self.name, REQUEST=req))
